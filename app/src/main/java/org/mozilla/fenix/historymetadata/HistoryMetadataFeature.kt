...
class HistoryMetadataFeature(
    ...
    override fun start() {
        job = scope.launch(Dispatchers.IO) {
            // Group the queried list of [HistoryMetadata] according to their search term.
            val historyMetadataMap = historyMetadataStorage.getHistoryMetadataSince(Long.MIN_VALUE)
                .filter { it.totalViewTime > 0 && it.key.searchTerm != null }
                .groupBy { it.key.searchTerm!! }

            val historyMetadata = mutableListOf<HistoryMetadataContainer>()

            for ((title, data) in historyMetadataMap) {
                historyMetadata.add(
